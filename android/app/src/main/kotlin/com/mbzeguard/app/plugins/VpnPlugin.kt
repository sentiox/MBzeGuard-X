...
import com.mbzeguard.app.core.Core
import com.mbzeguard.app.extensions.awaitResult
import com.mbzeguard.app.extensions.resolveDns
import com.mbzeguard.app.models.StartForegroundParams
import com.mbzeguard.app.models.VpnOptions
import com.mbzeguard.app.services.BaseServiceInterface
import com.mbzeguard.app.services.FlClashXService
import com.mbzeguard.app.services.FlClashXVpnService
...
data object VpnPlugin : FlutterPlugin, MethodChannel.MethodCallHandler {
    ...
    private val connectivity by lazy {
        MBzeGuardApplication.getAppContext().getSystemService<ConnectivityManager>()
    }
    ...
    private fun resolverProcess(
        ...
        if (!uidPageNameMap.containsKey(nextUid)) {
            uidPageNameMap[nextUid] =
                MBzeGuardApplication.getAppContext().packageManager?.getPackagesForUid(nextUid)
                    ?.first() ?: ""
        }
    ...
    private fun bindService() {
        if (isBind) {
            MBzeGuardApplication.getAppContext().unbindService(connection)
        }
        val intent = when (options?.enable == true) {
            true -> Intent(MBzeGuardApplication.getAppContext(), FlClashXVpnService::class.java)
            false -> Intent(MBzeGuardApplication.getAppContext(), FlClashXService::class.java)
        }
        MBzeGuardApplication.getAppContext().bindService(intent, connection, Context.BIND_AUTO_CREATE)
    }

    private fun showSubscriptionNotification(title: String, message: String, actionLabel: String, actionUrl: String) {
        val context = MBzeGuardApplication.getAppContext()
